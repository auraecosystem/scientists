...
export interface SemanticCreateRequest {
  ...
  output: { type: SemanticOutputType; schema?: Record<string, unknown> };
  constraints?: { provider?: SemanticProvider; localOnly?: boolean; maxTokens?: number; maxTurns?: number };
}
...
function uniqueModalities(inputs: SemanticModality[]): SemanticModality[] { return [...new Set(inputs)]; }
...
function validateRequest(request: SemanticCreateRequest): ValidationResult {
  ...
  return { valid: errors.length === 0, status: errors.length === 0 ? 'verified' : 'rejected', errors, warnings, evidence: { phase: 'create', normalizedInputs: normalized } };
}
...
export function validateSemanticExecution(request: SemanticCreateRequest, provider: SemanticProvider, session?: SmartLanguageSession | null): ValidationResult {
  ...
  return { valid: errors.length === 0, status: errors.length === 0 ? 'verified' : 'rejected', errors, warnings, evidence: { phase: 'validate', provider, capabilities, inputs, outputType: request.output.type, localOnly: request.constraints?.localOnly === true, sessionSource: session?.source ?? null } };
}

function toPrompt(subject: unknown): MultimodalPrompt { return typeof subject === 'string' ? subject : JSON.stringify(subject); }

function buildSessionOptions(request: SemanticCreateRequest): SessionOptions {
  const inputs = uniqueModalities(request.inputs);
  return { expectedInputs: inputs.map((type) => type === 'text' ? { type, languages: ['en'] } : { type }), expectedOutputs: [{ type: 'text', languages: ['en'] }], ...(request.constraints?.maxTokens ? { maxTokenBudget: request.constraints.maxTokens } : {}) };
}
...
function forceProvider(options: SmartCreateOptions, provider?: SemanticProvider): SmartCreateOptions {
  ...
  const globalObject = new Proxy(baseGlobal, { has(target, property) { return property === 'LanguageModel' ? false : property in target; } });
...
export async function createSemanticExecution(request: SemanticCreateRequest, options: SmartCreateOptions = {}): Promise<SemanticExecution> {
  ...
  const session = await SmartLanguageSession.create({
    ...
    options: { ...(options.options || {}), ...buildSessionOptions(request) },
  ...
  if (!validation.valid) { await session.destroy(); throw new Error(`Semantic validation rejected execution: ${validation.errors.join(' ')}`); }
...
export function validateSemanticResult(execution: SemanticExecution, result: string): ValidationResult {
  ...
  if (execution.request.output.type === 'json') {
    ...
    } catch { errors.push('Result is not valid JSON.'); }
  }
