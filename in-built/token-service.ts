...
class TokenService {
  ...
  private fallbackToMainThread(): void {
    ...
    this.worker = null;
    for (const pending of this.pendingRequests.values()) pending.reject(new Error('Token worker stopped; retrying on the main thread.'));
  ...
  async countPayloadTokens(messages: MultimodalMessage[]): Promise<number> {
    ...
    catch (error) { this.fallbackToMainThread(); return countTokensMainThread(messages).catch(() => { throw error; }); }
  }
  ...
  private countWithWorker(messages: MultimodalMessage[]): Promise<number> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try { this.worker?.postMessage({ id, messages }); }
      catch (error) { this.pendingRequests.delete(id); reject(error); }
    });
  }
