...
> [!CAUTION]
> **Caution:** On the web, sampling parameters are not available by default. However, an [origin trial](https://developer.chrome.com/origintrials/#/view_trial/4469259680211795969) unlocks the `samplingMode` parameter with preset enum values. For Chrome Extensions, legacy sampling parameters (`topK`, `temperature`) and `LanguageModel.params()` still exist. If you find parameter tuning helpful, let us know in the [GitHub repository](https://github.com/webmachinelearning/prompt-api/issues/170) or [Chrome Issue Tracker](https://g-issues.chromium.org/issues/new?component=1583624&pli=1&authuser=0) regarding API improvements to make these work reliably across varied models and versions.
...
When participating in the [sampling parameters origin
