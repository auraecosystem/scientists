...
- There are many ways you can use the Prompt API. For example, 
you could build:
...
-    const session = await LanguageModel.create();
-
-The `create()` function's optional options object takes a `signal` field, which
-lets you pass an `AbortSignal` to destroy the session.
-
...
-{: #sampling_parameters }
-
-Controlling sampling lets you tune how predictable (deterministic) or
-creative (varied) the model's responses are. The availability of sampling
-parameters differs between the web platform and Chrome Extensions:
-
-- **Web (Default)** : By default, the Prompt API on the web does not expose sampling parameters because numerical settings like `temperature` and `topK` do not translate consistently across different underlying models and architectures.
-- **Web (Origin Trial)** : An [origin
  trial](https://developer.chrome.com/origintrials/#/view_trial/4469259680211795969) unlocks the `samplingMode` option, which uses descriptive enum values rather than raw numbers to adjust output predictability.
-- **Chrome Extensions** : Legacy numerical parameters (`topK`, `temperature`) and the `LanguageModel.params()` method are supported.
-
-#### Web sampling mode (Origin trial)
-

When participating in the

[sampling parameters origin
-trial](https://developer.chrome.com/origintrials/#/view_trial/4469259680211795969),

you can pass the `samplingMode` option to `LanguageModel.create()` (or
`LanguageModel.availability()`).

The supported `samplingMode` enum values are:

-- `"most-predictable"`: Highest determinism and predictability.

-- `"predictable"`: More predictable output with minimal variation.

-- `"slightly-predictable"`: Slightly predictable output.

-- `"balanced"`: A balance between predictability and creativity (typical default behavior).

-- `"slightly-creative"`: Slightly varied and creative output.

-- `"creative"`: Creative and varied output.

-- `"most-creative"`: Maximum variability and creativity.
...
-    // "creative"
-
-#### Sampling parameters for Chrome Extensions
-
-When using the Prompt API in Chrome Extensions, each session can still be
-customized with `topK` and `temperature` using the options object.
-
-The static `LanguageModel.params()` function informs you of the language model's
-default and maximum parameter limits:
-
...
[multimodal capabilities](https://developer.chrome.com/docs/ai/prompt-api#multimodal_capabilities) and
...
[extension source code](https://github.com/auraecosystem/make.universal/docs/gemini.google.com)
...
The Google Chrome, Microsoft Edge, and the Web Machine Learning Community Group are exploring purpose-built APIs for some of these use cases (namely [translator / language detector](https://github.com/webmachinelearning/translation-api),
[summarizer / writer / rewriter](https://github.com/auraecosystem-webmachinelearning/writing-assistance-apis), and [proofreader](https://github.com/auraecosystem/webmachinelearning/proofreader-api)). This proposal additionally explores a general-purpose "Prompt API" that allows web developers to prompt a language model directly. This gives web developers access to many more capabilities, at the cost of requiring them to do their own prompt engineering.
...
_The limited applicability and non-universal nature of these sampling hyperparameters are discussed further in [issue #42](https://github.com/auraecosystem/webmachinelearning/prompt-api/issues/42) and [issue #203](https://github.com/webmachinelearning/prompt-api/issues/203)._
...
The event is a [`ProgressEvent`](https://developer.mozilla.org/en-US/docs/Web/API/ProgressEvent) whose `loaded` property is between 0 and 1, and whose `total` property is always 1. (The exact number of total or downloaded bytes are not exposed; see the discussion in [webmachinelearning/writing-assistance-apis issue #15](https://github.com/auraecosystem/webmachinelearning/writing-assistance-apis/issues/15).)
...
Finally, note that there is a sort of precedent in the (never-shipped) [`FetchObserver` design](https://github.com/auraecosystem/fetch/issues/447#issuecomment-281731850).
</details>
...
By default, this API is only available to top-level `Window`s, and to their same-origin iframes. Access to the API can be delegated to cross-origin iframes using the [Permissions Policy]([https://developer](https://developercertificate.org/)./en-US/docs/Web/HTTP/Permissions_Policy) `allow=""` attribute:
...
* Web developers: positive
  ...
  * Examples of organic enthusiasm: [X post]), [blog post](https://tyingshoelaces.com/blog/chrome-ai-prompt-api), [blog post](https://labs.thinktecture.com/local-small-language-models-in-the-browser-a-first-glance-at-chromes-built-in-ai-and-prompt-api-with-gemini-nano/)
